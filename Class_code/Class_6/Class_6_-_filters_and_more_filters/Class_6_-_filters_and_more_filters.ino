...
AudioFilterStateVariable filter1;        //xy=398,225
AudioFilterLadder        ladder1;        //xy=472,377
AudioFilterStateVariable filter2;        //xy=593,220
AudioMixer4              mixer2;         //xy=661,127
AudioEffectEnvelope      envelope1;      //xy=845,150
AudioAmplifier           amp1;           //xy=959,170
AudioOutputI2S           i2s1;           //xy=1206,128
...
AudioConnection          patchCord11(mixer2, envelope1);
AudioConnection          patchCord12(envelope1, amp1);
...
void loop() {
  ...
  if (current_time - prev_time[1] > rate1) {
    ...
    if (gate_enable == 1) {
      ...
      waveform2.frequency(freq1 * 1.006);
      envelope1.noteOn();
      arp_order_index++;
