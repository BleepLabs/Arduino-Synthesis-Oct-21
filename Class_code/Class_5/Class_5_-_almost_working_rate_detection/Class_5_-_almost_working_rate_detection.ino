...
unsigned long rate_detect[4];
unsigned long sample1, sample2;
int rate_index;
unsigned long lockout_time, lockout;
  ...
  if (peak1.available()) {
    ...
    if (prev_peak1_reading < .5 && peak1_reading > .5) {
      if (lockout == 0) {
        ...
        drum1.noteOn();
        sample1 = sample2;
        sample2 = current_time;
        Serial.println(sample2 - sample1);
      }
    }
  }

  if (current_time - lockout_time > 20) {
    lockout = 0;
  }
