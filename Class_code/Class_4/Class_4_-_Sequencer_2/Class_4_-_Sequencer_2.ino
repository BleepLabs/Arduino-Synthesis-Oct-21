...
AudioMixer4              mixer1;         //xy=271.0057067871094,196
AudioSynthWaveform       waveform3;      //xy=321.0056915283203,329
AudioSynthSimpleDrum     drum1;          //xy=387,391
AudioSynthSimpleDrum     drum2;          //xy=402,448
AudioFilterStateVariable filter1;        //xy=471.0057067871094,202
AudioEffectEnvelope      envelope1;      //xy=650.0056762695312,187
AudioMixer4              mixer2;         //xy=684,352
AudioAmplifier           amp1;           //xy=829,319
AudioOutputI2S           i2s1;           //xy=989.0056762695312,312
...
AudioConnection          patchCord9(envelope1, 0, mixer2, 0);
AudioConnection          patchCord10(mixer2, amp1);
AudioConnection          patchCord11(amp1, 0, i2s1, 0);
AudioConnection          patchCord12(amp1, 0, i2s1, 1);
AudioControlSGTL5000     sgtl5000_1;     //xy=1006.0056762695312,119
...
//here we'll put frequencies to play on the drum object
...
void setup() {
  ...
  drum1.frequency(440);
  drum1.length(250); //how long it takes for the sound to fade out in milliseconds
  drum1.pitchMod(.2); // above .5 the pitch increases, below it decreases. The farther from .5 the further the pitch goes
...
void loop() {
  ...
  //none of this is doing anything now
  ...
  //this part is still controlling things
  amp1.gain(potRead(3)); 
  ...
  if (current_time - prev_time[1] > seq1_rate) {
    ...
    if (seq1[seq_index] == 1) {  //both sequences use the same seq_index
      envelope1.noteOn();
    }

    if (seq2[seq_index] > 0) { //here the array has frequencies, not just 1 or 0
