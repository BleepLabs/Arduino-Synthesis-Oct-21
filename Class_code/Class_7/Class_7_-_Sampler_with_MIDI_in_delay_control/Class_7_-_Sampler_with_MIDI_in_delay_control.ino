...
void setup() {
  ...
  delay_mixer.gain(0, 1.0); //input
  delay_mixer.gain(1, 0); //feedback

  wet_dry_mixer.gain(0, .2);
  wet_dry_mixer.gain(1, .8);
...
void loop() {
  ...
  if (MIDI.read()) {
    ...
    if (dm_type == midi::NoteOn) {
      ...
      if (dm_velocity > 0) {
        midi_delay_len = map(dm_note, 40.0, 80.0, 0, float(delay_bank_length));
        float smapler_note = (dm_note / chromatic[53 - 12]) * 2.0;
        //sampler1.frequency(smapler_note);
        //sampler1.play();

        //   Serial.println(smapler_note);
      }
  ...
  if (current_time - prev_time[1] > 30 && 1) {
    prev_time[1] = current_time;
    for (int j = 0; j < 66; j++) {
      set_LED(j, j / 66.0, 1, 0);
    }
    set_LED(dm_note - 40, dm_note / 66.0, 1, 1);
    LEDs.show();
